...
struct KeychainStore {
    static func save(key: String, data: Data) throws {
        // Store both service and account attributes so command-line lookups with -s (service)
        // and -a (account) can find the item. Also set a reasonable accessibility.
        let query: [String: Any] = [kSecClass as String: kSecClassGenericPassword,
                                    kSecAttrService as String: key,
                                    kSecAttrAccount as String: key,
                                    kSecAttrAccessible as String: kSecAttrAccessibleAfterFirstUnlock,
                                    kSecValueData as String: data]
        // Remove any existing item with same service/account before adding
