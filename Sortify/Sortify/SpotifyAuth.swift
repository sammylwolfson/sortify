        ...
        URLSession.shared.dataTask(with: req) { [weak self] data, resp, err in
            ...
            do {
                ...
                do {
                    ...
                    // Log keychain save errors so we can diagnose why the token wasn't persisted
    ...
    private func loadTokenFromKeychain() {
        do {
            if let t = try KeychainStore.loadToken(key: keychainKey) {
                self.token = t
                if t.expiresAt > Date().timeIntervalSince1970 {
                    self.isSignedIn = true
                }
                // Debug: confirm token load at runtime (will appear in Console)
                if #available(macOS 11.0, *) {
                    os_log("DEBUG: Loaded token from keychain (service=%{public}@) accessPrefix=%{public}@ expiresAt=%{public}f", log: .default, type: .debug, keychainKey, String(t.accessToken.prefix(12)), t.expiresAt)
                    // Also print to stdout for immediate visibility when running the binary from terminal
                    print("DEBUG: Loaded token from keychain (service=\(keychainKey)) accessPrefix=\(t.accessToken.prefix(12)) expiresAt=\(t.expiresAt)")
                } else {
                    print("DEBUG: Loaded token from keychain (service=\(keychainKey)) accessPrefix=\(t.accessToken.prefix(12)) expiresAt=\(t.expiresAt)")
                }
            } else {
                if #available(macOS 11.0, *) {
                    os_log("DEBUG: No token found in keychain for service=%{public}@", log: .default, type: .debug, keychainKey)
                    print("DEBUG: No token found in keychain for service=\(keychainKey)")
                } else {
                    print("DEBUG: No token found in keychain for service=\(keychainKey)")
                }
            }
            ...
            if #available(macOS 11.0, *) {
                os_log("Keychain load error: %{public}@", log: .default, type: .error, String(describing: error))
...
private class PresentationProvider: NSObject, ASWebAuthenticationPresentationContextProviding {
    ...
    func presentationAnchor(for session: ASWebAuthenticationSession) -> ASPresentationAnchor {
        // Prefer the key window or first app window so ASWebAuthenticationSession has a valid anchor.
        if let key = NSApp.keyWindow { return key }
        if let first = NSApp.windows.first { return first }
        // Fallback to a new window (rare) to avoid returning an empty anchor which prevents the session UI from appearing.
        return NSWindow()
    }
}
