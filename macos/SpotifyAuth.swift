...
final class SpotifyAuth: ObservableObject {
        ...
        URLSession.shared.dataTask(with: req) { [weak self] data, resp, err in
            ...
            do {
                ...
                guard let access = json?["access_token"] as? String,
                      let expiresIn = json?["expires_in"] as? Double else { completion(.failure(AuthError.invalidTokenResponse)); return }
                let refresh = json?["refresh_token"] as? String
                let scope = json?["scope"] as? String
                let token = Token(accessToken: access, refreshToken: refresh, expiresAt: Date().timeIntervalSince1970 + expiresIn, scope: scope, tokenType: json?["token_type"] as? String)
    ...
    private func loadTokenFromKeychain() {
        do {
            if let data = try KeychainStore.load(key: keychainKey) {
                let decoder = JSONDecoder()
                let t = try decoder.decode(Token.self, from: data)
                self.token = t
    ...
    private func refreshAccessToken(refreshToken: String) async throws -> Token {
        ...
        let json = try JSONSerialization.jsonObject(with: data, options: []) as? [String: Any]
        guard let access = json?["access_token"] as? String,
              let expiresIn = json?["expires_in"] as? Double else { throw AuthError.invalidTokenResponse }
        let refresh = json?["refresh_token"] as? String // sometimes not returned
        let scope = json?["scope"] as? String
        let token = Token(accessToken: access, refreshToken: refresh ?? refreshToken, expiresAt: Date().timeIntervalSince1970 + expiresIn, scope: scope, tokenType: json?["token_type"] as? String)
        return token
    }
}
